 ...
 * Date: 2/4/2012
 */
X234 = function () { this.x6756 = []; this.animations = []; this.defaultValues = []; this.x8886 = 0; this.height = parseInt(jQuery("body").css("height"), 10); this.target = $(window); this.scrollSpace = this.height - this.target.innerHeight(); this.target.bind('scroll', { me: this }, this.x9874); }; X234.prototype.x7654 = function (x8886) {
    if (x8886 > 1) { x8886 = 1; } else if (x8886 < 0) { x8886 = 0; }
...
X234.prototype.x9874 = function (event) {
    var jaralax = event.data.me; var y = jaralax.target.scrollTop();; var x8886 = y / jaralax.scrollSpace; if (x8886 > 1) { x8886 = 1; }
    ...
    if (!object) { throw new Error("no values set."); }
    if (typeof object != "object") { throw new Error("wrong data type values. expected: 'object', got: '" + typeof object + "'"); }
    if (object.size == 0) { throw new Error("Got an empty values object"); }
    return true;
}
X234.prototype.x8754 = function (selector, values) {
    if (!selector) { throw new Error("no selector defined."); }
    if (X234.x8456(values)) { var newDefault = new X234Default(selector, values); newDefault.x9239(); this.defaultValues.push(newDefault); }
}
X234.prototype.x211334 = function (selector, values) {
    if (!selector) { throw new Error("no selector defined."); }
    if (X234.x8456(values)) { var newDefault = new X234Static(selector, values[0], values[1]); this.defaultValues.push(newDefault); }
}
X234.prototype.x12343 = function (selector, values) {
    ...
    if (X234.x8456(values)) {
        if (values[0] && values[1]) {
            ...
            else { throw new Error("no animation boundry found."); }
        }
        else { throw new Error("bad animation data."); }
    }
}; X234Default = function (selector, values) { this.selector = selector; this.values = values }
...
X234Static.prototype.x9239 = function (position) {
    var start; var end; if (this.start_values['x8886'].indexOf("%") >= 0) { start = parseInt(this.start_values['x8886'], 10) / 100; }
    if (this.end_values['x8886'].indexOf("%") >= 0) { end = parseInt(this.end_values['x8886'], 10) / 100; }
    if (x8886 > start && x8886 < end) {
        for (i in this.start_values) {
            if (i != "x8886") { $(this.selector).css(i, this.start_values[i]); }
        }
    }
}
...
X234Animation = function (selector, start_values, end_values) { this.selector = selector; this.start_values = start_values; this.end_values = end_values; }; X234Animation.prototype.x9239 = function (x8886) {
    var start; var end; var style
        ...
        for (i in this.start_values) {
            if (i != "x8886" && i != "style") {
                if (undefined != this.end_values[i]) {
                    var units = GetUnits(this.start_values[i] + ""); units = units.replace("-", "")
                ...
                else { $(this.selector).css(i, this.start_values[i]); }
            }
        }
    }
}; X234.x8127 = {
    "linear": function (currentTime, beginningValue, changeInValue, duration, power) { return currentTime / duration * changeInValue + beginningValue; }, "easeOut": function (currentTime, beginningValue, changeInValue, duration, power) {
